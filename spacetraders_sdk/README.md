# Rust API client for spacetraders_sdk
...
Put the package under your project folder in a directory named `spacetraders_sdk` and add the following to `Cargo.toml` under `[dependencies]`:

```
spacetraders_sdk = { path = "./spacetraders_sdk" }
```
